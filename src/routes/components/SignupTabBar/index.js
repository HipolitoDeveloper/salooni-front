...
import { saveEmployeeParse } from "../../../services/EmployeeService";
    ...
    try {
      ...
      if (procedures.length) {
        for (let procedure of procedures) {
          //TODO: SAVE EMPLOYEEID AT PROCEDURE
          procedure = { ...procedure, salonId: salonId };
          savedProcedures.push(await saveProcedureParse(procedure));
        }
      }

      owner.procedures = savedProcedures
      const { id: employeeId } = await saveEmployeeParse(owner, true);
      ...
      if (employees.length) {
        for (const employee of employees) {
          ...
          await saveEmployeeParse(employee);
        }
      }
      ...
      handleModal({ ...modal, visible: true, variant: "alert", errors: Errors.ERROR_MESSAGE });
    }
  };
